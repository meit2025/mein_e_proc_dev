    ...
    public function purchaseExport(Request $request)
    {
        ...
        $transformedData = $data->map(function ($pr) {
            return [
                ...
                'qty' => optional($pr->cashAdvancePurchases->unit)->qty ?? '',
                'unit_price' => optional($pr->cashAdvancePurchases->unit)->unit_price ?? '',
                'account_assignment' => optional($pr->cashAdvancePurchases->unit)->account_assignment_categories ?? '',
                'material_group' => optional($pr->cashAdvancePurchases->unit)->material_group ?? '',
                'material_number' => optional($pr->cashAdvancePurchases->unit)->material_number ?? '',
                'uom' => optional($pr->cashAdvancePurchases->unit)->uom ?? '',
                'tax' => optional($pr->cashAdvancePurchases->unit)->tax ?? '',
                'short_text' => optional($pr->cashAdvancePurchases->unit)->short_text ?? '',
            ];
        });
