...
const ListBusinessTrip: React.FC<Props> = ({ reimburses, users, types, currencies, csrf_token }) => {
  ...
  const handleOpenForm = (business_trip: BusinessTrip | null = null) => {
    setcurrentBusinesstrip(business_trip);
    setOpen(true);
  };

  const handleCloseForm = () => {
    setcurrentBusinesstrip(null);
    setOpen(false);
  };
  ...
  const columns: ColumnDef<BusinessTrip>[] = [
    ...
    },
    {
      ...
      header: 'Action',
      cell: ({ row }) => {
        const business_trip = row.original;
...
export default ListBusinessTrip;
