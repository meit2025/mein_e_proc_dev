...
import {
  DETAIL_MASTER_PURCHASING_GROUP,
  EDIT_MASTER_PURCHASING_GROUP,
...
const Update = ({ id }: { id: number }) => {
  ...
  const getdetail = useCallback(
    async () => {
      setIsLoading(true);
      try {
        const response = await axiosInstance.get(DETAIL_MASTER_PURCHASING_GROUP(props.id));
      ...
      <div className='card card-grid h-full min-w-full p-4'>
        <div className='card-body'>
          <FormMapping
            ...
            methods={methods}
            formModel={formModel}
            url={`${EDIT_MASTER_PURCHASING_GROUP}/${props.id}`}
            redirectUrl={LIST_PAGE_MASTER_PURCHASING_GROUP}
          />
        </div>
      </div>
    </>
...
Update.layout = (page: ReactNode) => (
  <MainLayout title='Purchasing Group' description='Purchasing Group Update'>
    {page}
  </MainLayout>
);
