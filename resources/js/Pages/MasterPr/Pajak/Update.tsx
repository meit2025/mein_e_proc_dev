...
import { DETAIL_MASTER_PAJAK, EDIT_MASTER_PAJAK } from '@/endpoint/pajak/api';
...
const Update = ({ id }: { id: number }) => {
  ...
  const getdetail = useCallback(
    async () => {
      setIsLoading(true);
      try {
        const response = await axiosInstance.get(DETAIL_MASTER_PAJAK(props.id));
      ...
      <div className='card card-grid h-full min-w-full p-4'>
        <div className='card-body'>
          <FormMapping
            ...
            methods={methods}
            formModel={formModel}
            url={`${EDIT_MASTER_PAJAK}/${props.id}`}
...
Update.layout = (page: ReactNode) => (
  <MainLayout title='TAX' description='Tax Update'>
    {page}
  </MainLayout>
);
