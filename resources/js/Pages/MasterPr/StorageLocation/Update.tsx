...
import {
  DETAIL_MASTER_STORAGE_LOCATION,
  EDIT_MASTER_STORAGE_LOCATION,
...
const Update = ({ id }: { id: number }) => {
  ...
  const getdetail = useCallback(
    async () => {
      setIsLoading(true);
      try {
        const response = await axiosInstance.get(DETAIL_MASTER_STORAGE_LOCATION(props.id));
      ...
      <div className='card card-grid h-full min-w-full p-4'>
        <div className='card-body'>
          <FormMapping
            ...
            methods={methods}
            formModel={formModel}
            url={`${EDIT_MASTER_STORAGE_LOCATION}/${props.id}`}
