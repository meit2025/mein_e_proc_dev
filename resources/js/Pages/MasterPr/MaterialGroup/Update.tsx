...
const Update = ({ id }: { id: number }) => {
  ...
  const getdetail = useCallback(
    async () => {
      setIsLoading(true);
      try {
        const response = await axiosInstance.get(DETAIL_MASTER_MATERIAL_GROUP(props.id));
      ...
      <div className='card card-grid h-full min-w-full p-4'>
        <div className='card-body'>
          <FormMapping
            ...
            methods={methods}
            formModel={formModel}
            url={`${EDIT_MASTER_MATERIAL_GROUP}/${props.id}`}
            redirectUrl={LIST_PAGE_MASTER_MATERIAL_GROUP}
          />
        </div>
      </div>
    </>
...
Update.layout = (page: ReactNode) => (
  <MainLayout title='Material Group' description='Material Group Update'>
    {page}
  </MainLayout>
);
