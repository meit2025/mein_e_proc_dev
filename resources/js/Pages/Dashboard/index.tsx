...
function Index(dataTotal: any) {
  ...
  useEffect(() => {
    ...
    const fetchFilteredData = async () => {
      const response = await axios.get(
        `/dashboard/filter/?status=${selectedStatus}&date=${selectedTime}`,
      );
  ...
  const total =
    (categories.waiting ?? 0) +
    (categories.process ?? 0) +
    (categories.approved ?? 0) +
    (categories.rejected ?? 0);
              ...
              <div className='card h-full'>
                <div className='card-header'>
                  <h3 className='card-title'>
                    {statusNames[selectedStatus] || 'Reimbursement'} Status
                  </h3>
                  <div className='menu'>
                ...
                <div className='card-body flex flex-col gap-4 p-5 lg:p-7.5 lg:pt-4'>
                  <div className='flex flex-col gap-0.5'>
                    <span className='text-sm font-normal text-gray-700'>
                      Status {statusNames[selectedStatus] || 'Reimbursement'}{' '}
                    </span>
      ...
      <div className='container-fixed'>
        <div className='card h-full'>
          ...
          <div className='card-body'>
            <DeclarationPieChart declaration={declaration} notDeclaration={notDeclaration} />
          </div>
        </div>
      </div>
    </>
